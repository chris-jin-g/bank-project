...
export function* handleDeposits() {
  ...
  const api = new ApiEndpoint();
  const token = auth.getToken();
  const requestURL = api.getDepositsPath();
  let depositsAmount=yield select(makeDepositsAmountSelector());
  let depositsCurrency = yield select(makeDepositsCurrency());
  let depositsCurrencyName = yield select(makeDepositsCurrencyName());
  let userID = yield select(makeUserID());
  console.log(depositsCurrency);
  console.log(depositsCurrencyName);
  console.log(depositsAmount);
  console.log(userID);
  const depositsURL=requestURL+"/paypal?amount="+depositsAmount+"&currency="+depositsCurrencyName+"&userID="+userID;
  if (!depositsAmount)
    return yield put(
      depositsErrorAction(<FormattedMessage {...messages.errorAmount} />),
    );
  else
      window.open(depositsURL,'_self');
