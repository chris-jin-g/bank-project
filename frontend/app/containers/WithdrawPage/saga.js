...
export function* handleWithdraw() {
  ...
  yield all([
    email ? call(handlePaypalAccount) : (email = null),
  ]);
  ...
  if (!withdrawAmount || email==null || paypalValidationFlag==0){
    if(!withdrawAmount)
      yield put(
        withdrawErrorAction(<FormattedMessage {...messages.errorWithdrawAmount} />),
      );
    else
      yield put(withdrawErrorAction(""),);
    if(email==null){
      return yield put(withdrawEmailErrorAction(<FormattedMessage {...messages.errorWithdrawEmail}/>),);
      console.log("withdraw email error")
    }        
    else{
      console.log("withdraw email success");
      yield put(withdrawEmailErrorAction(""),);
    }
    if(paypalValidationFlag==0){
      yield put(withdrawEmailErrorAction(<FormattedMessage {...messages.errorWithdrawEmailValidation}/>),);
    }
    else{
      yield put(withdrawEmailErrorAction(""),);  
    }
...
function* handlePaypalAccount() {
  const api = new ApiEndpoint();
  const email = yield select(makePaypalAccount());
  const requestURL = api.getIsEmailPath(email);
  const isEmail = /[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/;
  const limit = 255;
  yield put(enterNewEmailAction(email));
  ...
  if (!isEmail.test(email)) {
    paypalValidationFlag=0;
    return false;
  }
  else{
    paypalValidationFlag=1;
    return true;
  }
